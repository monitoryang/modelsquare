"""Chunked video upload endpoints for resumable uploads."""
...
async def init_chunked_upload(
    ...
    # For non-OWL models, check Triton deployment
    _is_owl = (
...
# ---------------------------------------------------------------------------
# 3. Complete upload (merge + start inference)
# ---------------------------------------------------------------------------
...
async def complete_chunked_upload(
    ...
    """Merge uploaded chunks and start video inference."""
    redis = await get_redis()
    meta = await _get_upload_meta(redis, upload_id)
    if meta is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Upload session not found or expired")
    ...
    # Generate task_id and merge chunks
    task_id = str(uuid.uuid4())
    video_path = os.path.join(tempfile.gettempdir(), f"video_input_{task_id}.mp4")
    ...
    try:
        ...
        if merged_size != meta["file_size"]:
            os.remove(video_path)
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail=f"Merged file size mismatch: expected {meta['file_size']}, got {merged_size}",
            )
    ...
    # Start inference via shared helper
    task_result = await _start_video_inference(
        task_id=task_id,
        model_id=model_id,
        model=model,
        ...
        conf_threshold=meta.get("conf_threshold", 0.25),
        iou_threshold=meta.get("iou_threshold", 0.45),
